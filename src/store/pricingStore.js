        ...
        for (const product of products) {
          ...
          const nextVariants = product.variants.map((variant) => {
            ...
            if (!key) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'unrecognized chain option',
              });
              return variant;
            }
            ...
            if (!target) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'no matching target pricing',
              });
              return variant;
            }
          ...
          if (missingVariants.length > 0) {
            missingSummaries.push({
              product,
              titles: missingVariants.map(({ title, reason }) =>
        ...
        for (const product of products) {
          ...
          const nextVariants = product.variants.map((variant) => {
            ...
            if (!signature.key) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'unrecognized chain or size',
              });
              return variant;
            }

            const target = targetByKey.get(signature.key);
            if (!target) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'no matching target pricing',
        ...
        for (const product of products) {
          ...
          const nextVariants = product.variants.map((variant) => {
            ...
            if (!identity.key) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'unrecognized band or size',
              });
              return variant;
            }
        ...
        for (const product of products) {
          ...
          const nextVariants = product.variants.map((variant) => {
            ...
            if (!signature.key) {
              missingVariants.push({
                title: variant?.title ?? `Variant ${variant?.id ?? 'unknown'}`,
                reason: 'unrecognized mode, chain, or size',
              });
              return variant;
            }
            ...
            const normalizedCurrentOptions = Array.isArray(variant.options)
              ? variant.options.map((option) => String(option ?? '').trim())
              : [];
            const normalizedTargetOptions = Array.isArray(target.options)
              ? target.options.map((option) => String(option ?? '').trim())
              : [];

            const optionsChanged = !arraysEqual(
              normalizedCurrentOptions,
              normalizedTargetOptions,
            );
            const priceChanged = variant.price !== target.price;
            const compareAtPriceChanged = variant.compareAtPrice !== target.compareAtPrice;

            if (variant.id && (priceChanged || compareAtPriceChanged || optionsChanged)) {
              if (!updatesByProduct.has(product.id)) {
                updatesByProduct.set(product.id, {
                  productId: product.id,
                  productTitle: product.title,
              ...
              if (priceChanged) {
                variantUpdate.price = target.price;
              }

              if (compareAtPriceChanged) {
                variantUpdate.compareAtPrice = target.compareAtPrice;
              }

              if (optionsChanged) {
                variantUpdate.options = normalizedTargetOptions;
              }

              updatesByProduct.get(product.id).variants.push(variantUpdate);
            }
            ...
            if (normalizedTargetOptions.length > 0) {
              nextVariant.options = normalizedTargetOptions;
            }
