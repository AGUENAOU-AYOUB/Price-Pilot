...
const MIN_SHOPIFY_REST_INTERVAL_MS = 500;
let lastMetafieldRequestAt = 0;
...
const enforceMetafieldRateLimit = async () => {
  const elapsed = Date.now() - lastMetafieldRequestAt;
  if (elapsed < MIN_SHOPIFY_REST_INTERVAL_MS) {
    await sleep(MIN_SHOPIFY_REST_INTERVAL_MS - elapsed);
  }
  lastMetafieldRequestAt = Date.now();
};
...
const fetchProductMetafields = async (productId) => {
  ...
  try {
    await enforceMetafieldRateLimit();

    const response = await fetch(METAFIELDS_ENDPOINT(productId), {
